...
rule bowtie2_global_align:
    ...
    params:
        ...
        index_prefix=lambda wildcards, input: (
            str(input.idx[0]).rsplit('.1.bt2', 1)[0] if isinstance(input.idx, list) and len(input.idx) > 0
            else str(input.idx)
        ),
    threads: 20
    resources:
...
rule merge_global_local:
    ...
    params:
        ...
        bam1=lambda wildcards, input: input[0] if isinstance(input, list) and len(input) > 0 else str(input),
        bam2=lambda wildcards, input: input[1] if isinstance(input, list) and len(input) > 1 else "",
    conda:
        "../envs/bowtie2_rescue.yml"
    shell:
        r"""
        ...
        echo "## {params.prefix}" > {output.mapstat}
        ...
        samtools view -c -F 4 {params.bam1} >> {output.mapstat}

        # If local bam is present, count mapped for local; else, print 0
        printf "local_${{tag}}\t"  >> {output.mapstat}
        if [[ -n "{params.bam2}" ]]; then
            samtools view -c -F 4 {params.bam2} >> {output.mapstat}
        else
